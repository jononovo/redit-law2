...
| Card eligibility | [docs.crossmint.com/agents/payment-methods/cards/enroll-card](https://docs.crossmint.com/agents/payment-methods/cards/enroll-card) | US-issued Visa or Mastercard credit/debit only. Not supported on Visa: non-US, business, prepaid, Chase, Fidelity. AMEX/Ramp need Crossmint approval. Surface in setup wizard. |
| Passkey-capable device for users | — | Crossmint's verification + authorization ceremonies use WebAuthn passkeys (separate layer from Firebase login — passkeys authorize the *card*, not the *user session*). Four fallback paths handled automatically by the browser, in cascade order: (1) **Google Password Manager / iCloud Keychain** passkey already synced to the user's Google/Apple account — one-tap; (2) **Laptop OS verification** — Windows Hello PIN, macOS account password, Linux passphrase; (3) **QR-to-phone** — laptop shows QR, user scans with phone camera, phone confirms with biometric or PIN; (4) **Biometric** — Touch ID, Face ID, fingerprint reader when available. No TOTP apps (Authy / Google Authenticator) — card networks reject TOTP for tokenization. Effectively zero users get stuck — every modern laptop has at least path #2. |
