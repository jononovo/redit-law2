...
- **Bot delete + Rail 3 Crossmint agent cleanup** — there is no `DELETE /api/v1/bots/:botId` route or UI today; bots are append-only. When/if we add bot deletion, the handler needs to (1) revoke any open Rail 3 OrderIntents for that bot, (2) call Crossmint `DELETE /agents/:agentId` (wrapper not written yet — would live in `features/payment-rails/rail3/agents.ts` next to `createAgent`), and (3) call existing `storage.deleteRail3AgentByBotId(botId)` to clear our `rail3_agents` row. Also probably worth cascading: revoke Rail 5 cards, tear down tunnel, delete pending messages, etc. Not built now because there's no caller and orphaned Crossmint agents are harmless (Crossmint allows multiple per user). Context: `project_knowledge/currently_building/rail3/rail3-open-points.md` §5.
