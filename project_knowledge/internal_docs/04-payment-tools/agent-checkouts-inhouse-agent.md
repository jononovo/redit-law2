---
name: agent-checkouts-inhouse-agent
description: "Captain Crunch — the auto-provisioned in-house agent that buys from any store via Crossmint's Agent Checkout API, paying with the owner's virtual cards. Technical plan + feature doc."
created: 2026-07-13
last_updated: 2026-07-13
---

# Agent Checkouts — Captain Crunch (in-house agent)

> **Every account gets a first-party agent, "Captain Crunch", that can buy from almost any store: the owner pastes a product URL + plain-language instructions on the `/inhouse-agent` page, picks one of their virtual cards, and Crossmint's Agent Checkout API drives a remote browser through the merchant's checkout. When the remote agent asks for payment, our server mints a one-time merchant-locked PAN from the selected Rail 3 card and submits it — card numbers never touch the browser.**
...
- Dashboard presence: listed as an agent (with in-house badge) on `/agents` and the overview, **without** replacing the "add your own agent" CTA.

## Why It Exists (Tier 3 — owner approval required to change)

The platform's pitch is "give your agent spending power" — but most signups don't have an agent yet. Captain Crunch closes that gap: it demonstrates the full virtual-card loop (vault → mint → merchant-locked one-time PAN → receipt) using only first-party infrastructure, and doubles as the reference integration for Crossmint Agent Checkouts. The public name is one constant (`INHOUSE_AGENT_NAME`) because "Captain Crunch" may need a rename before public launch (Quaker trademark).

## Relationship to the Master Agent plan

This feature is the **first incarnation** of the in-house-agent thesis from `_payment_build_ideas/260528_rail3-master-agent-plan.md` (converged direction 2026-06-04), using Crossmint's hosted Agent Checkout runtime. It does **not** retire that plan: a self-hosted browser-use master agent may still be built later (owner-confirmed 2026-07-13) and would reuse the surfaces built here (the `/inhouse-agent` observance page, the `agent_checkouts` table, the form-factor-agnostic engine API). Deliberate v1 divergences from that doc's converged direction (owner-approved 2026-07-13, in-conversation):
- **Rail 3 only in v1** (not Rail 1 + Rail 3): the Crossmint checkout runtime takes card payment; stablecoin flows stay out of scope.
- **Dashboard page, not a separate tenant**: `/inhouse-agent` inside the existing dashboard; the engine (`features/payment-rails/agent-checkouts/`) stays form-factor-agnostic so other surfaces (CLI, tenant) can consume the same API later.
...
```
Owner (on /inhouse-agent page)                CreditClaw server                        Crossmint
  1. pick card, paste URL + request  ──────▶  POST /api/v1/agent-checkouts
                                              ensure buyer profile ─────────────────▶  POST /buyer-profiles (once per owner)
                                              create checkout ────────────────────▶   POST /agent-checkouts {target, buyerProfileId, constraints}
  2. page polls every ~2s ─────────────────▶  GET /api/v1/agent-checkouts/:id
  ...
  3. non-card questions (OTP, choices) ────▶  POST /api/v1/agent-checkouts/:id/actions/:actionId
...
### A. Constants + provisioning

1. `lib/inhouse-agent.ts` — `INHOUSE_AGENT_NAME = "Captain Crunch"`, `INHOUSE_BOT_TYPE = "inhouse"`, short description string. `agentPlatform` stays **null** — `botType === "inhouse"` is the sole discriminator ("creditclaw" is not in the closed `AGENT_PLATFORMS` enum and would render as a raw string in platform pills). Route is name-agnostic: `/inhouse-agent`.
2. **Lazy, race-safe get-or-create** in `GET /api/v1/bots/mine` (the one endpoint both target pages already call — it is hit 4× in parallel on overview, so this MUST be conflict-safe AND cheap):
   - Schema: partial unique index on `bots` — `uniqueIndex("bots_inhouse_owner_uidx").on(bots.ownerUid).where(sql\`bot_type = 'inhouse'\`)`.
   - `storage.ensureInhouseBot(ownerUid, ownerEmail)`: **select-first** (matching the actual rail3 precedent `app/api/v1/rail3/cards/route.ts:67-81` — no write on the hot path when the row exists); on miss, insert with `.onConflictDoNothing()` then re-select for the race.
   - Insert shape = the pre-claimed variant from `app/api/v1/bots/register/route.ts:144-150`: `walletStatus: "active"`, `ownerUid`, `claimedAt: now`, `claimToken: null`, `botType: "inhouse"`, `agentPlatform: null`, plus `apiKeyHash`/`apiKeyPrefix` from a generated-and-discarded key (bcrypt hash satisfies notNull; nothing external ever holds the key).
   ...
   - **Separate response key — the in-house bot is NOT in the `bots` array.** `bots/mine` returns it as `inhouse_agent: {...}` alongside `bots`/`pending_pairings` (same pattern as `pending_pairings`). Rationale: six existing consumers treat every `bots[]` row as a linkable external agent (link-bot dialogs ×3, add-card picker, rail5 wizard, orders filter) and would otherwise offer card/wallet linking to an agent whose API key doesn't exist. With its own key, only the agents page and overview render it — deliberately.

### B. Schema (all additive — safe `db:push` on local → Replit dev → prod)

3. `agent_checkouts` table: `id` serial PK, `checkoutId` text notNull unique (ours, `achk_` + hex — generator alongside `features/payment-rails/rail3/ids.ts`), `crossmintCheckoutId` text unique, `ownerUid` notNull, `botId` notNull, `cardId` notNull, `productUrl` notNull, `request` text notNull, `merchantContext` text, `maxCostCents` integer, `status` text notNull default `"created"` (`created | running | awaiting_user_action | succeeded | failed | cancelled`), `rail3TransactionId` text, `receipt` jsonb, `lastEvent` text, `createdAt`/`updatedAt`. Index on `ownerUid`.
4. `owners.crossmintBuyerProfileId` text (nullable) — one buyer profile per owner, created on first checkout from the default shipping address (`storage.getDefaultShippingAddress`) + owner email. No default address → 422 `shipping_address_required` (page links to shipping settings).
...
7. `service.ts` — the only stateful logic:
   ...
   - `syncCheckout(row, jwt)`: Crossmint GET → map status → **if pending action's `responseSchema` requests card fields** (detect by field names, e.g. `cardNumber`/`number` + `cvc`): run card gates again, `fetchOneTimeCredentials`, `createRail3Transaction(status: "credentials_issued", botId: inhouseBot)`, submit action values, never return card fields to the client. **Any other action**: persist status and return `{ pending_user_action: { id, expires_at, response_schema } }` for the page to render. On `succeeded`: `updateRail3Transaction(status: "charged", amountCents from receipt)` + fire-and-forget `recordOrder` (fields per `app/api/v1/bot/rail3/confirm/route.ts:36-49`, plus `productUrl`).

### D. API routes — owner-auth conventions from `app/api/v1/rail3/cards/route.ts`

8. `app/api/v1/agent-checkouts/route.ts` — `POST` (zod: `card_id`, `product_url` (url), `request` (1–2000), `merchant_context?` (≤2000), `max_cost_cents?` (int ≥1)); `GET` — owner's checkout list, newest first, for the history panel.
9. `app/api/v1/agent-checkouts/[checkoutId]/route.ts` — `GET` = sync/poll (above); `DELETE` = cancel (Crossmint DELETE, tolerate 404 like `revokeOrderIntent`).
10. `app/api/v1/agent-checkouts/[checkoutId]/actions/[actionId]/route.ts` — `POST` `{ values }` → submit non-card user action.
...
12. `app/(dashboard)/inhouse-agent/page.tsx` + `components/inhouse-agent/` — **this is an observance page**: once a checkout starts, the remote-session playback is the page. Layout mirrors Crossmint's own demo (progress rail left, large "Agent browser session" viewport center, instruction banner above).
    ...
    - `checkout-history.tsx` — list from `GET /api/v1/agent-checkouts`.
...
13. `components/inhouse-agent/inhouse-agent-card.tsx` — a **dedicated card component** (BotCard is untouched: it has no botType prop and threading one through both pages' local interfaces is needless churn). Styled identically to BotCard (same container/pill/status classes), "IN-HOUSE" badge, links to `/inhouse-agent`, no settings/webhook affordances. Rendered explicitly by the agents page and overview from the `inhouse_agent` response key. The "add your own agent" CTA card stays as-is; agent counts do NOT include the in-house agent.
14. `components/dashboard/sidebar.tsx` — add to `mainNavItems`: `{ icon: Bot, label: INHOUSE_AGENT_NAME, href: "/inhouse-agent", tag: "beta" }`.

### F. Tests (per `tests/_README.md` — business logic only; add a Current Coverage row)

15. `tests/agent-checkouts/`:
    ...
    - DB: `ensureInhouseBot` race (two concurrent inserts → one row, partial unique index holds); `agent_checkouts` storage round-trip.

### G. Docs & deploy checklist

16. This document (update Status when built) + add a row to the folder table in `payments_overview.md` ("What's in this folder") — that table, not frontmatter scanning, is this folder's actual index convention.
...
18. `.env.example`: add `CROSSMINT_AGENT_CHECKOUT_CLIENT_KEY` under a `# --- CrossMint (Agent Checkouts) ---` section. **Owner action before deploy:** create/scope the key in the Crossmint console (agent-checkouts + buyer-profiles scopes, production) and add it to Replit Secrets.
19. Schema (all additive) — but **`npm run db:push` hangs on a pre-existing rail5_transactions prompt** (see `.agents/memory/db-push-and-test-baseline.md`): apply locally via direct `psql` SQL and keep `shared/schema.ts` in sync; hand the same SQL to the owner to run on Replit dev + prod **before** the code deploy.
...
| Area | Path |
|---|---|
| Constants | `lib/inhouse-agent.ts` |
| Crossmint client | `features/payment-rails/agent-checkouts/client.ts` |
| Service | `features/payment-rails/agent-checkouts/service.ts` |
| Buyer profile | `features/payment-rails/agent-checkouts/buyer-profile.ts` |
| Routes | `app/api/v1/agent-checkouts/**` |
| Page | `app/(dashboard)/inhouse-agent/page.tsx`, `components/inhouse-agent/*` |
| Provisioning | `server/storage/core.ts` (`ensureInhouseBot`), `app/api/v1/bots/mine/route.ts` |
| Schema | `shared/schema.ts` (`agentCheckouts`, `owners.crossmintBuyerProfileId`, bots partial unique index) |
...
- Non-card payment methods (Shop Pay, bank transfer), sticky sessions / login-gated merchants, passing card up-front (Crossmint ships this later), webhooks, background checkout runner, per-card rail3 guardrail evaluation (table exists, still unconsumed), buyer-profile editing UI (recreate by clearing the column).

## Status

**Built 2026-07-14 — not yet deployed.** Feature complete and verified locally (typecheck clean, 11 tests passing). Payment confirmed submit-only against Crossmint docs. Pending before deploy: (1) Crossmint client key with agent-checkouts + buyer-profiles scopes (`CROSSMINT_AGENT_CHECKOUT_CLIENT_KEY`); (2) schema applied to Replit dev + prod (`agent_checkouts` table, `owners.crossmint_buyer_profile_id`, `owners.default_agent_checkout_card_id`, `bots_inhouse_owner_uidx`); (3) naming ("Captain Crunch") trademark check; (4) first real checkout logs the full payload (`AGENT_CHECKOUT_DEBUG=1`) to discover the live-view stream + exact card-action field names.
