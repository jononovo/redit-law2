...
| 6 | `server/storage/payment-links.ts` | Remove the 7 payment-link methods (lines 20-86): `createPaymentLink`, `getPaymentLinksByBotId`, `getPaymentLinkByStripeSession`, `getPaymentLinkByPaymentLinkId`, `getPaymentLinksByOwnerUid`, `updatePaymentLinkStatus`, `completePaymentLink`. Remove `paymentLinks` table import and `PaymentLink`/`InsertPaymentLink` type imports from line 3-4. Trim drizzle-orm import (line 8): remove `desc` and `inArray` (only used by dead methods; remaining methods only use `eq`, `and`, `sql`, `gte`). Rename Pick type from `PaymentLinkMethods` → `PairingWaitlistMethods` with only the 6 kept method names. Rename export from `paymentLinkMethods` → `pairingWaitlistMethods`. | **CRITICAL — DO NOT DELETE THIS FILE.** It also contains 6 LIVE methods: `createPairingCode` (used by `pairing-codes/route.ts`), `getPairingCodeByCode` (used by `pairing-codes/status/route.ts`), `claimPairingCode` (used by `bots/register/route.ts`), `getRecentPairingCodeCount` (used by `pairing-codes/route.ts`), `addWaitlistEntry` (used by `waitlist/route.ts`), `getWaitlistEntryByEmail` (used by `waitlist/route.ts` and `addWaitlistEntry` itself). Keep `pairingCodes` and `waitlistEntries` table imports. Keep `IStorage` import (used by `addWaitlistEntry`'s `this` cast won't be needed after rename, but keep for safety). |
| 7 | `server/storage/index.ts` | Rename import: `paymentLinkMethods` → `pairingWaitlistMethods` (line 4). Rename spread: `...paymentLinkMethods` → `...pairingWaitlistMethods` (line 34). Update import path if file renamed (file stays as `payment-links.ts` for now — rename is optional cosmetic). | Keep the import/spread — the file still provides live pairing code and waitlist methods. |
...
6. `server/storage/payment-links.ts` (**NOT deleted** — remove 7 dead payment-link methods, keep 6 live pairing/waitlist methods, rename export)
7. `server/storage/index.ts` (rename import/spread to match)
