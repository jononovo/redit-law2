...
| `managed-agents/` | Managed agents — remote runtimes CreditClaw orchestrates on the owner's behalf (vs user-linked bots). Overview + the Crossmint Agent Checkout runtime (“Jennifer”, the concierge shopping agent), which pays with Rail 3 virtual cards. |
