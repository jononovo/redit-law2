...
`components/onboarding/onboarding-wizard-v2.tsx`. Flow: choose-agent-type → register-agent → sign-in → claim-token (fallback only) → add-card-bridge. The wizard serves both new-customer onboarding (anonymous) and in-app "Add agent" (signed in); the signed-in path short-circuits most of the step array (see below).

- **register-agent** (`steps/register-agent-with-code.tsx`): creates a pairing code via `authFetch` (`POST /api/v1/pairing-codes`, `getSessionUser` — cookie + Bearer). Signed-in users get an owner-linked code (ownerUid at creation); anonymous users get an anonymous, IP-rate-limited code. Generation is deduped with a module-level in-flight promise (StrictMode double-mount previously created an anonymous twin code). Shows the code in `BotInstructionBlock` ("Register at creditclaw.com/SKILL.md" + "Use code: xxx-xxx"), and polls `GET /pairing-codes/status` every 5s via `authFetch` (the status route also uses `getSessionUser`, since owned codes 404 for non-owners) — auto-advances when the agent registers. Skippable.
