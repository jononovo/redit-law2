...
`components/onboarding/onboarding-wizard-v2.tsx`. Flow: choose-agent-type → register-agent → sign-in → claim-token (fallback only) → add-card-bridge. Serves new-customer onboarding; in-app "Add agent" now has its own wizard at `/add-agent` (see below). The signed-in short-circuit remains for signed-in users who land on `/onboarding` directly.

- **register-agent** (`steps/register-agent-with-code.tsx`): creates a pairing code via `authFetch` (`POST /api/v1/pairing-codes`, `getSessionUser` — cookie + Bearer). Signed-in users get an owner-linked code (ownerUid at creation); anonymous users get an anonymous, IP-rate-limited code. The wizard passes the chosen `agentPlatform` and it is stored on the pairing code (`pairing_codes.agent_platform`, validated against `AGENT_PLATFORMS`); the register endpoint copies it to `bots.agent_platform` at creation, so the platform tag survives delayed registrations that complete after the user leaves the wizard. Generation is deduped with a module-level in-flight promise (StrictMode double-mount previously created an anonymous twin code). Shows the code in `BotInstructionBlock` ("Register at creditclaw.com/SKILL.md" + "Use code: xxx-xxx"), and polls `GET /pairing-codes/status` every 5s via `authFetch` (the status route also uses `getSessionUser`, since owned codes 404 for non-owners) — auto-advances when the agent registers. Skippable.
- **Signed-in short-circuit:** when the user is authenticated and a pairing code exists, Continue on register-agent jumps straight to add-card-bridge — no sign-in, no claim-token, no claim call. The owner-linked code activates the bot the moment it registers (see code lifecycle). If registration is detected while still on the step, the wizard marks the bot connected, clears the stored code, and jumps to add-card-bridge. The pending code shows as a "Waiting for agent" card on the agents/overview pages (`GET /api/v1/bots/mine` → `pending_pairings`).
- **Pairing hook** (`components/onboarding/use-onboarding-pairing.ts`): owns all pairing-code state and handlers per the wizard hook convention — sessionStorage persistence (exported `PAIRING_CODE_STORAGE_KEY`), the one-shot claim effect (fires once the user is signed in and `claimEligible` is true), toasts, and outcome callbacks (`onAgentLinked` / `onCodeAdopted` / `onClaimFallback`). The orchestrator only wires callbacks to step navigation. `finishOnboarding` clears the stored code so a later wizard run can't resurface a stale one.
- **Claim (anonymous path, both orders work):** `POST /api/v1/pairing-codes/claim` after sign-in. A `registered` code links + activates the waiting bot; a still-`pending` code is **adopted** (ownerUid set) so the agent activates the moment it registers later. Claiming a code you already own is an idempotent `adopted`.
- **Fallback:** claim failure or skipping register drops the user onto the manual claim-token step. Agents registering with an unowned code still receive a claim_token (owned codes don't — they activate immediately).
- **Code lifecycle:** `pending` → `registered` (bot exists but inert: walletStatus pending, no owner) → `claimed`. These three are the only statuses (the legacy `paired` status and its writer/readers were removed). Owner-created codes (ownerUid at creation) skip the inert stage — registering with one activates immediately. Since the signed-in short-circuit, this is the normal path for in-app agent additions; the inert stage only occurs for anonymous onboarding.

### Add Agent Wizard (live at `/add-agent`)

`components/onboarding/add-agent-wizard.tsx`, page at `app/add-agent/page.tsx` (auth-gated: signed-out users redirect to `/onboarding`). The in-app "Add Agent" buttons on the agents and overview pages link here; public/marketing links (hero, docs, allowance) still point to `/onboarding`. Flow: choose-agent-type → register-agent → claim-token (reachable only via Skip). No card steps — the add-card-bridge/Rail5 chain belongs to onboarding only; cards are added from the dashboard. Differences from V2: no sign-in step and no claim hook (owner-linked codes auto-activate on registration); own sessionStorage key (`ADD_AGENT_PAIRING_CODE_STORAGE_KEY` = `creditclaw_add_agent_pairing_code`) so it never collides with the onboarding wizard's code; finish (registration detected, Continue, or claim-token resolution/skip) navigates to `/agents` with no `POST /owners/onboarded` — a still-pending code surfaces there as the "Waiting for agent" card. Platform tag: carried by the pairing code on the register path (see V2 register-agent note); `saveAgentPlatform` PATCH is used only on the claim-token path, where no pairing code exists.
