...
`components/onboarding/add-agent-wizard.tsx`, page at `app/add-agent/page.tsx` (auth-gated: signed-out users redirect to `/onboarding`). The in-app "Add Agent" buttons on the agents and overview pages link here; public/marketing links (hero, docs, allowance) still point to `/onboarding`. Flow: choose-agent-type → register-agent → claim-token (reachable only via Skip) → add-card-bridge, then chains `Rail5SetupWizardContent` inline. Differences from V2: no sign-in step and no claim hook (owner-linked codes auto-activate on registration); own sessionStorage key (`ADD_AGENT_PAIRING_CODE_STORAGE_KEY` = `creditclaw_add_agent_pairing_code`) so it never collides with the onboarding wizard's code; finish navigates to `/agents` with no `POST /owners/onboarded`. The bridge's Back button returns to whichever step the user came from (`bridgeReturnStep`: register-agent on the normal path, claim-token via the fallback). Platform tag: carried by the pairing code on the register path (see V2 register-agent note); `saveAgentPlatform` PATCH is used only on the claim-token path, where no pairing code exists.
