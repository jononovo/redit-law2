...
## Unified Scan · Three Tenant Surfaces

There is **one scan pipeline**, **one set of Perplexity calls**, **one scoring engine**, **one skill generator**, and **one `brand_index` row per domain**. Whether the scan is triggered from shopy.sh, brands.sh, the CreditClaw admin queue, or the public API, the work done server-side is identical. Every scan produces both: (a) a numeric ASX score with full pillar/signal breakdown, and (b) a generated `SKILL.md` + `skill.json`. Nothing is ever "scanned for shopy" vs "scanned for brands" — those are presentation concerns, not pipeline concerns.

What differs per tenant is **which facets of the same row each surface emphasizes**:
- **shopy.sh** — landing page is a leaderboard of `brand_index` ordered by `overallScore` (with a Recent Scores toggle ordering by `last_scanned_at`). Scanner form on `/agentic-shopping-score` accepts a domain, calls `POST /api/v1/scan`, and renders the score-centric result. Brand detail pages live at `/brands/[slug]` and lead with the score, pillar breakdown, and improvement suggestions. The generated skill is *available* but not the headline.
- **brands.sh** — the same `brand_index` is presented as a skill registry. Catalog at `/skills`, detail at `/skills/[vendor]`, sector pages at `/c/[sector]`. The score is shown as a quality signal, but the headline is the SKILL.md / skill.json a developer or agent can copy. Filter bar lets users browse by sector, tier, brand type, capability, etc.
- **CreditClaw** — the same `brand_index` powers the agent-facing `/api/v1/recommend` endpoint and `/api/v1/bot/skills` search. Scans are typically triggered via the admin scan queue (`/admin123/scan-queue`) rather than user submission. The score gates which merchants get recommended; the skill is what gets returned to the agent for execution.

Practical implication when adding features: **never branch the scan pipeline per tenant**. If a tenant needs a new piece of data, add it once to the pipeline → write it to `brand_index` → expose it through the relevant tenant surface. Tenant-specific logic belongs in `components/tenants/{tenant}/` and tenant-specific API routes, not in `features/brand-engine/`.
...
Uses Privy server wallets on Base chain, USDC funding via Stripe Crypto Onramp, and x402 payment protocol. **Modularized under `features/payment-rails/rail1/`:**
  ...
  - Webhook: `STRIPE_WEBHOOK_SECRET_ONRAMP` env var, event type `crypto.onramp_session.updated`. Balance sync endpoint: `POST /api/v1/usdc-wallet/balance/sync` with 30-sec cooldown and `reconciliation` transaction type for discrepancies. Schema includes `last_synced_at` column on `privy_wallets`.
...
- **Frontend:** Transfer button on both USDC Wallet and Card Wallet pages, dialog with destination picker (own wallets across both rails or external address), amount input in USD
...
- All rail dashboard pages (USDC Wallet, Card Wallet, Self-Hosted Cards) use these centralized endpoints. No rail-specific approval endpoints remain.
...
- **Rail wiring** (order creation fires ONLY after confirmed execution, never on pending requests):
  - Rail 1: `features/agent-interaction/approvals/rail1-fulfillment.ts` (approved) + `app/api/v1/usdc-wallet/bot/sign/route.ts` (auto-approved)
...
- `/solutions/usdc-wallet`: USDC Wallet landing page (public)
