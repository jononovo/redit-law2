...
**Crypto:** AES-256-GCM. The stored source is `base64(ciphertext‖tag)` with the 16-byte tag inline — exactly what WebCrypto's `subtle.decrypt` expects, so decrypt the decoded blob directly. `/bot/rail5/key` also returns `tag_hex` but it is the same bytes; no strip/reattach needed (v1 did this redundantly). Key/IV arrive as lowercase hex.

**v2 compat lessons (don't regress):** the shipped v4 skills are the contract — don't rename bridge message fields, MAIN-world globals, or result-shape keys; when in doubt read the skills, not the extension. Skill-requested fill names differ from record keys, so canonicalization must stay. Page-bridged setup must stay (pairing is agent-driven from page JS) — rejecting page-sender setup breaks pairing. But `api_base` must NEVER be settable from a tab (options page only): a page-settable backend URL redirects the credential-bearing key fetch to an attacker server.
