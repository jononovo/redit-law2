...
import type { ManagedAgentCheckout } from "@/shared/schema";
...
export async function authorizeAgentCheckoutRequest(
  ...
  let row: ManagedAgentCheckout | null = null;
  if (opts.checkoutId) {
    row = await storage.getManagedAgentCheckoutByCheckoutId(opts.checkoutId);
...
// Shared error → JSON envelope mapping for all /api/v1/managed-agents/checkouts routes.
export function mapAgentCheckoutError(err: unknown, label: string): NextResponse {
  ...
  if (err instanceof CrossmintApiError) {
    console.error(`[ManagedAgentCheckout] Crossmint error (${label}):`, err.status, err.body);
    return NextResponse.json(
      { error: "crossmint_error", message: err.message },
      { status: err.status >= 400 && err.status < 600 ? err.status : 502 }
    );
  }
  console.error(`[ManagedAgentCheckout] ${label} failed:`, err);
