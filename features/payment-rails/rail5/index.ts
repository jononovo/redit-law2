...
export { RAIL5_TEST_CHECKOUT_PAGE_ID, RAIL5_TEST_CHECKOUT_URL, RAIL5_AGENT_TEST_CHECKOUT_URL } from "./constants";
...
export function buildSpawnPayload(params: {
  ...
  return {
    task: [
      ...
      "5. Read creditclaw/CHECKOUT-GUIDE.md for browser checkout instructions. Use the decrypted card number and CVV to complete checkout at the merchant URL.",
      `6. Call POST /api/v1/bot/rail5/confirm with { "checkout_id": "${params.checkoutId}", "status": "success" } when done.`,
      `7. If checkout fails, call confirm with { "checkout_id": "${params.checkoutId}", "status": "failed" } instead.`,
      "8. Discard all decrypted card data immediately.",
      `9. Announce the result: "Purchase of ${params.itemName} at ${params.merchantName} — SUCCESS" or "— FAILED".`,
    ].join("\n"),
    cleanup: "delete",
