...
import type { AgentCheckout, Rail3Card } from "@/shared/schema";
...
import { generateAgentCheckoutId } from "./ids";
...
export function mapCardActionProps(action: CrossmintPendingUserAction): Record<string, CardSlot> | null {
  ...
  if (unmapped.length > 0) {
    console.error(`[AgentCheckout] card action has unmappable required fields: ${unmapped.join(", ")}`);
    return null;
  }
  return mapping;
}
...
export async function startCheckout(params: {
  ...
  const cm = await unwrapCrossmint<CrossmintCheckout>(res, "createAgentCheckout");

  return storage.createAgentCheckout({
    checkoutId: generateAgentCheckoutId(),
...
export async function syncCheckout(row: AgentCheckout, jwt: string): Promise<SyncResult> {
  ...
  if (process.env.AGENT_CHECKOUT_DEBUG === "1") {
    console.log(`[AgentCheckout] ${row.checkoutId} payload:`, redactCardData(JSON.stringify(cm)));
  } else {
    console.log(
      `[AgentCheckout] ${row.checkoutId} status=${cm.status} keys=${Object.keys(cm).join(",")} action=${cm.pendingUserAction?.id ?? "none"}`,
    );
  }
  ...
  if (status === "succeeded") {
    ...
    const claimed = await storage.claimAgentCheckoutSuccess(row.checkoutId);
    if (!claimed) {
      const current = (await storage.getAgentCheckoutByCheckoutId(row.checkoutId)) ?? row;
  ...
  const updated = (await storage.updateAgentCheckout(row.checkoutId, {
    status,
    lastEvent,
...
async function handleCardAction(row: AgentCheckout, jwt: string, cm: CrossmintCheckout): Promise<SyncResult> {
  ...
  const claimed = await storage.claimAgentCheckoutCardMint(row.checkoutId, action.id);
  if (!claimed) {
    const current = (await storage.getAgentCheckoutByCheckoutId(row.checkoutId)) ?? row;
    return { row: current, pendingUserAction: null, cardActionUnmappable: false };
  }
  ...
  const card = await assertCardUsable(claimed.cardId, claimed.ownerUid).catch(async (err) => {
    ...
    await storage.updateAgentCheckout(claimed.checkoutId, { status: "awaiting_user_action" });
    throw err;
  });
  ...
  try {
    ...
    const updated = (await storage.updateAgentCheckout(claimed.checkoutId, {
      status: "running",
    ...
    await storage.updateAgentCheckout(claimed.checkoutId, {
      status: "awaiting_user_action",
      answeredActionId: action.id,
...
async function finalizeSuccess(row: AgentCheckout, cm: CrossmintCheckout, lastEvent: string | null): Promise<AgentCheckout> {
  ...
  }).catch((err) => console.error("[AgentCheckout] recordOrder failed:", err));

  // claimAgentCheckoutSuccess already set status — persist only the payload.
  return (
    (await storage.updateAgentCheckout(row.checkoutId, {
...
export async function submitUserAction(row: AgentCheckout, jwt: string, actionId: string, values: Record<string, unknown>): Promise<AgentCheckout> {
  ...
  return (
    (await storage.updateAgentCheckout(row.checkoutId, { status: "running", lastEvent: "Answer submitted" })) ?? row
  );
}

// ─── Cancel ──────────────────────────────────────────────────────────────────
export async function cancelCheckout(row: AgentCheckout, jwt: string): Promise<AgentCheckout> {
  ...
  return (
    (await storage.updateAgentCheckout(row.checkoutId, { status: "cancelled", lastEvent: "Cancelled by owner" })) ?? row
  );
}
