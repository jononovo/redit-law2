...
| 3 | Payment Tools | Outbound rails, segmented by what the owner sees in the sidebar: **Rail 1** (Privy stablecoin wallet + x402, "USDC Wallet" tile, live), **Rail 3** (Crossmint Card Permissions — vaulted real card + N virtual cards as orderIntents, "Virtual Cards" tile, live; see `project_knowledge/internal_docs/04-payment-tools/rail3-virtual-cards.md`), **Rail 5** (self-hosted encrypted real cards, "Self-hosted Cards · Encrypted" tile, live). **Rail 2** (Crossmint USDC wallet for Worldstore, "Shop Wallet" tile) was an exploration and is dormant — likely retired since Rail 1 covers stablecoin/x402 directly. Module overview + shared `components/wallet/` conventions: `project_knowledge/internal_docs/04-payment-tools/payments_overview.md`. |
