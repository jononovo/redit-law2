...
- **Card Color Persistence:** Each card (Rail 4 and Rail 5) stores its own `card_color` (`purple`, `dark`, `blue`, `primary`). New cards get a random color on creation. Users can change it from the card detail page (Rail 5: color picker circles below card visual). Existing cards without a saved color use a stable hash of the card ID as fallback. The `CARD_COLORS` array and `resolveCardColor()` utility live in `components/wallet/types.ts`.
