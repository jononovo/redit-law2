...
- `master.ts` — master-level guardrail evaluation (fetches config, aggregates cross-rail spend from Rails 1, 2, and 5, calls `evaluateGuardrails`). Spend aggregation lives in `server/storage/master-guardrails.ts` — returns `{ rail1, rail2, rail5, total }` in micro-USDC. Rail 5 amounts are stored in cents and converted (× 10,000). Status filters: Rails 1/2 use `NOT IN ('failed')`, Rail 5 uses `IN ('approved', 'completed')`. Per-rail spend functions use the same filters for consistency.
