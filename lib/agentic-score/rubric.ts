...
export function computeScoreFromRubric(
  ...
  overallScore: number;
  breakdown: Record<string, { score: number; max: number; signals: { key: string; label: string; score: number; max: number; detail: string }[] }>;
} {
  const breakdown: Record<string, { score: number; max: number; signals: { key: string; label: string; score: number; max: number; detail: string }[] }> = {};

  let overallScore = 0;

  for (const pillar of rubric.pillars) {
    const signals: { key: string; label: string; score: number; max: number; detail: string }[] = [];
