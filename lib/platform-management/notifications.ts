...
export async function notifyPurchase(
  ...
  const { sendPurchaseAlertEmail } = await import("@/lib/platform-management/email");
...
export async function notifyBalanceLow(
  ...
  const { sendBalanceLowEmail } = await import("@/lib/platform-management/email");
...
export async function notifySuspicious(
  ...
  const { sendSuspiciousActivityEmail } = await import("@/lib/platform-management/email");
  const amountUsd = (amountCents / 100).toFixed(2);
