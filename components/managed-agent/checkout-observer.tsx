...
import { UserActionModal } from "@/components/managed-agent/user-action-modal";
import { isTerminalAgentCheckoutStatus, formatMmSs } from "@/lib/managed-agent-checkouts";
import { CheckoutStatusPill } from "@/components/managed-agent/status-pill";

// snake_case shape returned by /api/v1/managed-agents/checkouts (serializeAgentCheckout).
...
export function CheckoutObserver({ checkout, onExit }: CheckoutObserverProps) {
  ...
  const handleCancel = async () => {
    ...
    setCancelling(true);
    try {
      const res = await authFetch(`/api/v1/managed-agents/checkouts/${checkoutId}`, { method: "DELETE" });
  ...
  const handleActionSubmit = async (values: Record<string, unknown>) => {
    ...
    try {
      const res = await authFetch(`/api/v1/managed-agents/checkouts/${checkoutId}/actions/${actionId}`, {
        method: "POST",
