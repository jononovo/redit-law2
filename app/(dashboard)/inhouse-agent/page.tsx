...
import { INHOUSE_AGENT_NAME, INHOUSE_AGENT_DESCRIPTION } from "@/lib/inhouse-agent";
import { CheckoutForm } from "@/components/inhouse-agent/checkout-form";
import { CheckoutHistory } from "@/components/inhouse-agent/checkout-history";
import { CheckoutObserver, type AgentCheckoutData } from "@/components/inhouse-agent/checkout-observer";
...
export default function InhouseAgentPage() {
  const { user } = useAuth();
  ...
  const fetchHistory = useCallback(async () => {
    try {
      const res = await authFetch("/api/v1/agent-checkouts");
      if (res.ok) {
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <div className="flex items-center gap-2">
            <h2 className="text-lg font-bold text-neutral-900">{INHOUSE_AGENT_NAME}</h2>
            <span
          ...
          <p className="text-sm text-neutral-500">{INHOUSE_AGENT_DESCRIPTION}</p>
        </div>
      </div>
