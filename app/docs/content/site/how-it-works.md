...
- **USDC Wallet (Rail 1)** — USDC wallet on Base chain, funded via Stripe. Supports x402 protocol for autonomous agent payments.
- **Card Wallet (Rail 2)** — USDC wallet for shopping at merchants like Amazon via CrossMint smart wallets.
- **Self-hosted Cards (Rail 5)** — End-to-end encrypted card files. Disposable sub-agents decrypt, pay, and are deleted.
