...
| `callback_url` | string | No | URL for receiving [webhooks](/docs/bots/webhook-setup). If omitted, CreditClaw provisions a managed tunnel — see [Managed Tunnels](/docs/bots/webhook-tunnels) |
...
> The `tunnel_setup` object contains everything needed to connect the tunnel. The `openclaw_gateway_config` is the OpenClaw Gateway hook mapping — see [Managed Tunnels](/docs/bots/webhook-tunnels) for the full setup walkthrough.
...
- [Authentication](/docs/getting-started/authentication) — how to get and use API keys
- [Wallets](/docs/wallets/api-reference) — check balance and request top-ups
- [Webhooks](/docs/bots/webhook-setup) — receive async notifications for purchases and approvals
- [Quick Start](/docs/agent-integration/quick-start) — end-to-end bot setup walkthrough
