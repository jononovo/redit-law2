...
import { linkBotToEntity } from "@/lib/platform-management/agent-management/bot-linking";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await linkBotToEntity("rail2", Number(wallet_id), bot_id, user.uid);
    ...
    console.error("POST /api/v1/card-wallet/link error:", error);
