...
export async function PATCH(request: NextRequest) {
  ...
  await storage.ensureManagedAgent(auth.user.uid, email, CROSSMINT_CHECKOUT_RUNTIME);

  await storage.setManagedAgentDefaultCard(auth.user.uid, CROSSMINT_CHECKOUT_RUNTIME, cardId);
  const managed = await storage.getManagedAgent(auth.user.uid, CROSSMINT_CHECKOUT_RUNTIME);
